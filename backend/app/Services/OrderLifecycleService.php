...
class OrderLifecycleService
{
    ...
    public function transitionBySeller(SellerOrder $sellerOrder, int $sellerId, string $status, ?string $trackingNumber = null): SellerOrder
    {
        return DB::transaction(function () use ($sellerOrder, $sellerId, $status, $trackingNumber) {
            ...
            if (in_array($status, ['in-transit', 'delivered'], true)) {
                $shipment = $this->updateShipment($locked, $status, $trackingNumber);
    ...
    private function updateShipment(SellerOrder $sellerOrder, string $status, ?string $trackingNumber): Shipment
    {
        $shipment = Shipment::query()->firstOrNew(['seller_order_id' => $sellerOrder->id]);
        ...
        TrackingEvent::create([
            ...
            'status' => $status,
            'note' => $status === 'delivered' ? 'Seller marked this shipment as delivered.' : 'Shipment is in transit.',
            'occurred_at' => now(),
        ]);
    ...
    private function synchronizeOrder(int $orderId): Order
    {
        ...
        $status = match (true) {
            ...
            $statuses->contains(fn (string $value) => in_array($value, ['in-transit', 'picked-up', 'shipped'], true)) => 'in-transit',
