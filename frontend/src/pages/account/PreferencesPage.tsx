  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 py-6 md:px-8 lg:px-12">
      ...
      {loading ? <div className="rounded-sm border border-[var(--color-border)] bg-white p-8 text-sm text-[var(--color-ink-muted)]">Loading preferences...</div> : <div className="space-y-5">
        ...
        <section className="rounded-sm border border-[var(--color-border)] bg-white px-6 py-2"><h2 className="pt-4 text-sm font-[600]">Privacy and data</h2>{booleanRow("analytics_cookies_enabled", "Analytics cookies", "Help improve Marketo using aggregated usage data.")}{booleanRow("marketing_cookies_enabled", "Marketing cookies", "Allow personalized marketing measurement.")}</section>
      </div>}
    </div>
  );
}
