...
import AuthLayout, { Field, AuthAlert } from "./AuthLayout";
...
export default function ForgotPasswordPage({ onNavigate }: { onNavigate: NavFn }) {
  ...
  const resend = () => {
    setResent(false);
    setLoading(true);
    setTimeout(() => { setLoading(false); setResent(true); }, 800);
  };
  ...
  if (sent) {
    return (
      <AuthLayout
        ...
        subtitle={<>We sent a password reset link to <strong className="text-[var(--color-ink)]">{email}</strong></>}
        footer={
          <span>
        ...
        <div className="text-center py-2">
          ...
          {resent && <div className="mb-4"><AuthAlert type="success" message="A new reset link has been sent." /></div>}
          ...
          <div className="mt-4 pt-4 border-t border-[var(--color-border)]">
            <p className="text-xs text-[var(--color-ink-muted)]">
              ...
              <button onClick={() => onNavigate("reset-password")} className="text-[var(--color-navy)] font-[500] hover:underline cursor-pointer">
                Reset password
              </button>
            </p>
          </div>
