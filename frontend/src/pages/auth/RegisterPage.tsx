...
export default function RegisterPage({ onNavigate }: { onNavigate: NavFn }) {
  ...
  const safeReturn =
    requestedReturn.startsWith("/") && !requestedReturn.startsWith("//")
      ? requestedReturn
      : null
  const isCourierApplication = safeReturn === "/courier/apply"
  ...
  const submit = async (event?: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const redirectWithReturn = safeReturn
        ? `${verificationRoute}${
            verificationRoute.includes("?") ? "&" : "?"
          }returnTo=${encodeURIComponent(safeReturn)}`
        : verificationRoute
      navigate(redirectWithReturn, {
        replace: true,
        state: {
  ...
  if (success) {
    return (
      <AuthLayout
        ...
        subtitle={message ?? "We sent a verification code to your inbox."}
      >
        ...
        <span>
          ...
          <button
            onClick={() =>
              safeReturn
                ? navigate(
                    `/auth/login?returnTo=${encodeURIComponent(safeReturn)}`,
                  )
                : onNavigate("login")
            }
            className="text-[var(--color-navy)] font-[500] hover:underline cursor-pointer"
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        <div>
          <label className="block text-xs font-[600] text-[var(--color-ink)] mb-1.5">
            Phone number{" "}
            <span className="text-[var(--color-red)] ml-0.5">*</span>
          </label>
          <div
            className={`flex items-center rounded-sm border bg-white transition-all ${
              errors.phone
                ? "border-[var(--color-red)] focus-within:ring-2 focus-within:ring-[var(--color-red)]/15"
        ...
        <div className="space-y-2">
          <Field
            ...
            hint={
              !errors.password
                ? "Use 8-16 characters with uppercase, number, and symbol."
                : undefined
            }
            maxLength={16}
          />
        ...
        <div>
          ...
          {errors.agreed && (
            <p className="text-xs text-[var(--color-red)] mt-1.5 ml-6">
              {errors.agreed}
            </p>
          )}
        </div>
