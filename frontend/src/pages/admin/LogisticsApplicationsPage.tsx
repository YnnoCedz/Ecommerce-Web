...
export default function LogisticsApplicationsPage() {
  ...
  return <div className="flex h-full min-h-[calc(100vh-56px)]">
    ...
    <ConfirmDialog open={confirmApproval} title="Approve Logistics provider?" description="This creates an active provider and grants provider-manager capability to the applicant's existing Maketo identity." confirmLabel="Approve application" loading={saving} onCancel={() => setConfirmApproval(false)} onConfirm={() => void review("approve")} />
  </div>
}
