...
function Overview({ data, goTo }: { data: AdminAnalyticsSectionData; goTo: (tab: AdminAnalyticsTab) => void }) {
  ...
  return <div className="space-y-5">
    <AnalyticsKpis metrics={data.kpis} definitions={[
      ...
      { key: "active_buyers", label: "Purchasing accounts" }, { key: "active_sellers", label: "Active sellers" }, { key: "order_completion_rate", label: "Order completion rate", format: (value) => `${value}%` },
    ]}/>
    ...
    <div className="grid gap-5 lg:grid-cols-2"><AnalyticsPanel title="Order Status" subtitle="Current period distribution"><BreakdownBars values={data.order_status ?? {}}/></AnalyticsPanel><AnalyticsPanel title="Marketplace Account Growth" subtitle="New legacy buyer-role and seller-role accounts; administrators excluded"><TrendChart points={data.marketplace_growth ?? []} series={[["buyers", "Buyer-role accounts"], ["sellers", "Seller-role accounts"]]}/></AnalyticsPanel></div>
...
function UsersSellers({ data }: { data: AdminAnalyticsSectionData }) {
  return <div className="space-y-5"><SectionIntro title="Users & sellers" text="Marketplace participation, seller decisions, and compliance."/><AnalyticsKpis metrics={data.kpis} definitions={[{ key: "active_buyers", label: "Purchasing accounts" }, { key: "active_sellers", label: "Active sellers" }, { key: "seller_applications", label: "Seller applications" }, { key: "approval_rate", label: "Approval rate", format: percent }, { key: "rejection_rate", label: "Rejection rate", format: percent }, { key: "seller_deactivations", label: "Seller deactivations" }]}/><div className="grid gap-5 lg:grid-cols-2"><AnalyticsPanel title="Legacy Role Account Growth"><TrendChart points={data.user_growth ?? []} series={[["buyers", "Buyer-role accounts"], ["sellers", "Seller-role accounts"]]}/></AnalyticsPanel><AnalyticsPanel title="Seller Application Decisions"><TrendChart points={data.application_trend ?? []} series={[["approved", "Approved"], ["rejected", "Rejected"]]}/></AnalyticsPanel></div><AnalyticsPanel title="Seller Document Compliance"><BreakdownBars values={data.document_compliance ?? {}}/></AnalyticsPanel></div>
}
