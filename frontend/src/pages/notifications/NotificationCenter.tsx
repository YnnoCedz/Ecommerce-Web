...
import { Bell, Boxes, Gift, MessageSquare, Package, Settings2, ShieldAlert, Truck, UserCircle2, type LucideIcon } from "lucide-react";
...
const CATEGORY_CONFIG: Record<NotifCategory, CategoryConfig> = {
  orders: { label: "Orders", icon: Package, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  delivery: { label: "Delivery", icon: Truck, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  messages: { label: "Messages", icon: MessageSquare, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  account: { label: "Account", icon: UserCircle2, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  promotions: { label: "Promotions", icon: Gift, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  system: { label: "System", icon: Settings2, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  moderation: { label: "Moderation", icon: ShieldAlert, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
  inventory: { label: "Inventory", icon: Boxes, badgeClass: "bg-[var(--color-surface)] text-[var(--color-ink-muted)]", iconClass: "text-[var(--color-ink-muted)]" },
};
...
function resolveActionRoute(notification: NotificationRecord, role?: string | null) {
...
function NotificationRow({ notification, role, onOpen, onDismiss }: NotificationRowProps) {
  const category = isNotifCategory(notification.category) ? notification.category : "system";
  const cfg = CATEGORY_CONFIG[category];
  const unread = !notification.read_at;

  return (
    <div className={`flex gap-4 px-5 py-4 border-b border-[var(--color-border-subtle)] last:border-0 transition-colors ${unread ? "bg-[var(--color-navy-surface)]/35" : ""}`}>
      <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${cfg.badgeClass}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-3 mb-0.5">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-disabled)] whitespace-nowrap">{formatTime(notification.created_at)}</span>
            {unread && <div className="w-2 h-2 rounded-full bg-[var(--color-navy)] shrink-0" />}
          </div>
        </div>

        <p className="text-xs text-[var(--color-ink-muted)] leading-relaxed line-clamp-2 mb-2">{notification.body}</p>
...
export default function NotificationCenter() {
  ...
  const categoryOptions = useMemo(() => {
    const options: Array<{ id: string; label: string; count: number; unread: number }> = [
      {
        id: "all",
        label: "All notifications",
        count: meta?.total_count ?? notifications.length,
        unread: meta?.unread_count ?? notifications.filter((notification) => !notification.read_at).length,
      },
    ];
    ...
    const dynamicCategories = meta?.categories?.length
      ? meta.categories
      : Object.entries(
          notifications.reduce<Record<string, { count: number; unread: number }>>((acc, notification) => {
            const current = acc[notification.category] ?? { count: 0, unread: 0 };
            current.count += 1;
          ...
          category,
          label: isNotifCategory(category) ? CATEGORY_CONFIG[category].label : category.replace(/[-_]/g, " "),
          count: counts.count,
  ...
  const visibleNotifications = activeCategory === "all"
    ? notifications
    : notifications.filter((notification) => notification.category === activeCategory);

  const unreadCount = meta?.unread_count ?? notifications.filter((notification) => !notification.read_at).length;
  const activeCategoryLabel = activeCategory === "all"
    ? "All Notifications"
    : categoryOptions.find((item) => item.id === activeCategory)?.label ?? activeCategory;
  ...
  const handleDismiss = async (notification: NotificationRecord) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Unable to dismiss notification.");
    } finally {
      setNotificationBusy(notification.id, false);
    }
  };
    ...
    try {
      ...
      if (meta) {
        setMeta({
          ...meta,
          unread_count: 0,
          categories: meta.categories.map((item) => ({ ...item, unread_count: 0 })),
        });
      }
    } catch (err) {
  ...
  const handleDismissVisible = async () => {
    ...
    setBulkBusy(true);
    try {
      await Promise.all(visibleNotifications.map((notification) => dismissNotification(notification.id)));
      setNotifications((current) => current.filter((notification) => !ids.includes(notification.id)));
      setMeta((current) => {
        if (!current) return current;

        const dismissedUnread = visibleNotifications.filter((notification) => !notification.read_at).length;
        const nextCategories = current.categories
          .map((item) => {
            const dismissedCount = visibleNotifications.filter((notification) => notification.category === item.category).length;
            const dismissedUnreadCount = visibleNotifications.filter((notification) => notification.category === item.category && !notification.read_at).length;
            if (!dismissedCount) return item;

            return {
              ...item,
              count: Math.max(0, item.count - dismissedCount),
              unread_count: Math.max(0, item.unread_count - dismissedUnreadCount),
            };
          })
          .filter((item) => item.count > 0);
      ...
      setError(err instanceof Error ? err.message : "Unable to dismiss notifications.");
    } finally {
      setBulkBusy(false);
    }
  };
    ...
    <div className="bg-[var(--color-ground)] min-h-full">
      <div className="max-w-screen-xl mx-auto px-4 md:px-8 lg:px-12 py-6">
        <div className="flex items-center gap-2 mb-5">
          <button onClick={() => navigate("/")} className="font-[var(--font-mono)] text-[11px] text-[var(--color-ink-muted)] hover:text-[var(--color-navy)] cursor-pointer">
            Home
          </button>
          ...
          <span className="font-[var(--font-mono)] text-[11px] text-[var(--color-ink)]">{heading}</span>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
          <div>
            ...
            <p className="text-sm text-[var(--color-ink-muted)] mt-1">Your live notification feed is now powered by the backend.</p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              {bulkBusy ? "Updating..." : `Dismiss ${activeCategory === "all" ? "all" : "category"}`}
            </button>
          </div>
        </div>
              ...
              {categoryOptions.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    <Bell size={14} className="shrink-0 text-[var(--color-ink-muted)]" />
                    <span className="flex-1 text-sm font-[500]">{item.label}</span>
                    {item.unread > 0 ? (
                      <span className="font-[var(--font-mono)] text-[9px] px-1.5 py-0.5 rounded-full bg-[var(--color-navy)] text-white">{item.unread}</span>
                    ) : item.count > 0 ? (
                      <span className="font-[var(--font-mono)] text-[9px] text-[var(--color-ink-disabled)]">{item.count}</span>
                    ) : null}
                  </button>
                );
              })}
              ...
              {categoryOptions.map((item) => {
                ...
                return (
                  <button
                    ...
                    {item.unread > 0 && <span className={`font-[var(--font-mono)] text-[9px] px-1 rounded-full ${isActive ? "bg-white/20" : "bg-[var(--color-navy)] text-white"}`}>{item.unread}</span>}
                  </button>
                );
              })}
            </div>
            ...
            <div className="bg-white border border-[var(--color-border)] rounded-sm overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[var(--color-border)] bg-[var(--color-surface)]">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-[600] text-[var(--color-ink)]">{activeCategoryLabel}</h3>
                  <span className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-disabled)]">({visibleNotifications.length})</span>
                </div>
                {visibleNotifications.some((notification) => !notification.read_at) && (
                  <button onClick={handleMarkAllRead} disabled={bulkBusy} className="text-xs text-[var(--color-navy)] hover:underline cursor-pointer disabled:opacity-50">
                    Mark all read
                  </button>
                )}
              </div>
              ...
              {loading ? (
                <div className="py-16 text-center text-sm text-[var(--color-ink-muted)]">Loading notifications...</div>
                ...
                <div className="py-16 text-center">
                  ...
                  <p className="text-sm text-[var(--color-ink-muted)]">
                    {activeCategory === "all" ? "No notifications to show." : `No ${activeCategoryLabel.toLowerCase()} notifications.`}
                  </p>
                </div>
              ) : (
                <div>
