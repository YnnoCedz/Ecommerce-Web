...
const FULFILLMENT_CONFIG: Record<FulfillmentStatus, { label: string; color: string; bg: string; border: string }> = {
  new: { label: "New order", color: "var(--color-amber)", bg: "var(--color-amber-light)", border: "var(--color-amber-border)" },
  confirmed: { label: "Confirmed", color: "var(--color-navy)", bg: "var(--color-navy-surface)", border: "var(--color-navy-border)" },
  preparing: { label: "Preparing", color: "var(--color-navy)", bg: "var(--color-navy-surface)", border: "var(--color-navy-border)" },
  ready: { label: "Ready", color: "var(--color-green)", bg: "var(--color-green-light)", border: "var(--color-green-border)" },
  "picked-up": { label: "Picked up", color: "var(--color-amber)", bg: "var(--color-amber-light)", border: "var(--color-amber-border)" },
...
function normalizeStatus(status: string): FulfillmentStatus {
  return (status in FULFILLMENT_CONFIG ? status : "new") as FulfillmentStatus;
}
      ...
      {flow.map((status, index) => {
        ...
        return (
          <div key={status} className="flex items-center">
            <div className={`flex items-center gap-1 px-2 py-1 rounded-sm text-[9px] font-[var(--font-mono)] whitespace-nowrap ${done ? "bg-[var(--color-green-light)] text-[var(--color-green)]" : active ? "" : "text-[var(--color-ink-disabled)]"}`} style={active ? { background: cfg.bg, color: cfg.color } : {}}>
            ...
            {index < flow.length - 1 && <div className={`w-4 h-px ${done ? "bg-[var(--color-green)]" : "bg-[var(--color-border)]"}`} />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className={`bg-white border rounded-sm overflow-hidden transition-colors ${status === "new" ? "border-[var(--color-amber-border)]" : "border-[var(--color-border)]"}`}>
      ...
      <div className="flex items-center gap-4 px-5 py-3.5 border-b border-[var(--color-border)] bg-[var(--color-surface)]">
        <div>
          <p className="font-[var(--font-mono)] text-[11px] font-[600] text-[var(--color-ink)]">{order.order_number ?? `Order #${order.id}`}</p>
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-[500] text-[var(--color-ink)] truncate">{order.buyer?.name ?? "Guest buyer"}</p>
          <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-muted)] truncate">{order.buyer?.email ?? "No email"}</p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <StatusBadge status={status} />
          <span className="font-[var(--font-display)] text-base font-[400] text-[var(--color-ink)]">PHP {order.grand_total.toLocaleString()}</span>
          <button onClick={() => setExpanded((value) => !value)} className="w-6 h-6 flex items-center justify-center text-[var(--color-ink-muted)] hover:text-[var(--color-ink)] cursor-pointer transition-colors">
      ...
      <div className="flex gap-4 px-5 py-3 border-b border-[var(--color-border-subtle)]">
        <div className="flex gap-2">
          {order.items.map((item) => (
            <div key={item.id} className="w-12 h-12 bg-[var(--color-surface)] rounded-sm overflow-hidden shrink-0">
              <img src={item.image ?? "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=100&h=100&fit=crop&auto=format"} alt={item.product_name} className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
        <div className="flex-1 min-w-0">
          {order.items.map((item) => (
            <p key={item.id} className="text-xs text-[var(--color-ink)] truncate leading-relaxed">
              {item.product_name} <span className="text-[var(--color-ink-muted)]">{item.variant_name ? `(${item.variant_name})` : ""} x {item.quantity}</span>
            </p>
          ))}
        </div>
      </div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm text-[var(--color-ink-secondary)]">{order.shipping_address ?? "No shipping address"}</p>
            </div>
            <div>
              <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-muted)] uppercase tracking-wide mb-1.5">Payment</p>
              <p className="text-sm text-[var(--color-ink)]">{order.payment_method ?? "Unknown"}</p>
              <p className="text-sm font-[600] text-[var(--color-ink)]">PHP {order.grand_total.toLocaleString()}</p>
            </div>
            {order.courier && (
              <div>
                <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-muted)] uppercase tracking-wide mb-1.5">Courier assignment</p>
                <p className="text-sm font-[500] text-[var(--color-ink)]">{order.courier.name ?? "Courier pending"}</p>
                {order.courier.driver && <p className="text-sm text-[var(--color-ink-secondary)]">Driver: {order.courier.driver}</p>}
                <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-muted)] mt-0.5">Tracking: {order.courier.tracking ?? "N/A"}</p>
              </div>
            )}
            <div>
              ...
              {order.items.map((item) => (
                <div key={item.id} className="flex justify-between text-sm mb-0.5">
                  <span className="text-[var(--color-ink-secondary)] truncate mr-2">{item.product_name} x {item.quantity}</span>
                  <span className="font-[500] text-[var(--color-ink)] shrink-0">PHP {(item.unit_price * item.quantity).toLocaleString()}</span>
                </div>
              ))}
            </div>
          </div>
...
export default function SellerOrdersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === "new") {
      ...
      next = next.filter((order) => ["confirmed", "preparing", "ready", "picked-up", "in-transit"].includes(normalizeStatus(order.status)));
    } else if (activeFilter === "completed") {
      next = next.filter((order) => ["delivered", "completed"].includes(normalizeStatus(order.status)));
    } else if (activeFilter === "cancelled") {
      next = next.filter((order) => ["cancelled", "failed"].includes(normalizeStatus(order.status)));
    }
  ...
  const filters = [
    ...
    { id: "new", label: "New", count: orders.filter((order) => normalizeStatus(order.status) === "new").length },
    { id: "active", label: "Active", count: orders.filter((order) => ["confirmed", "preparing", "ready", "picked-up", "in-transit"].includes(normalizeStatus(order.status))).length },
    { id: "completed", label: "Completed", count: orders.filter((order) => ["delivered", "completed"].includes(normalizeStatus(order.status))).length },
    { id: "cancelled", label: "Cancelled / Failed", count: orders.filter((order) => ["cancelled", "failed"].includes(normalizeStatus(order.status))).length },
  ] as const;

  const newCount = filters[1].count;

  if (loading) {
    return <div className="px-6 py-6 text-sm text-[var(--color-ink-muted)]">Loading orders...</div>;
  }
    ...
    <div className="px-6 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          {newCount > 0 && <p className="text-sm text-[var(--color-amber)] mt-0.5 font-[500]">{newCount} new {newCount === 1 ? "order" : "orders"} require your attention</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "In transit", value: orders.filter((order) => ["picked-up", "in-transit"].includes(normalizeStatus(order.status))).length.toString(), sub: "Active shipments" },
          { label: "Completed", value: orders.filter((order) => ["delivered", "completed"].includes(normalizeStatus(order.status))).length.toString(), sub: "Fulfilled orders" },
          { label: "Revenue", value: `PHP ${orders.reduce((sum, order) => sum + order.grand_total, 0).toLocaleString()}`, sub: "All tracked orders" },
        ].map((card) => (
          <div key={card.label} className={`bg-white border rounded-sm p-4 ${card.urgent ? "border-[var(--color-amber-border)] bg-[var(--color-amber-light)]" : "border-[var(--color-border)]"}`}>
            <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-muted)] uppercase tracking-wide mb-1">{card.label}</p>
            <p className={`font-[var(--font-display)] text-2xl font-[300] ${card.urgent ? "text-[var(--color-amber)]" : "text-[var(--color-ink)]"}`}>{card.value}</p>
            <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-muted)]">{card.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by order ID, customer or product..." className="w-full pl-8 pr-3 py-2 text-sm bg-white border border-[var(--color-border)] rounded-sm text-[var(--color-ink)] placeholder:text-[var(--color-ink-disabled)] outline-none focus:border-[var(--color-navy)] transition-colors" />
        </div>
      </div>
      ...
      <div className="flex gap-0 border-b border-[var(--color-border)] mb-5">
        {filters.map((filter) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-[500] border-b-2 transition-colors cursor-pointer whitespace-nowrap ${activeFilter === filter.id ? "border-[var(--color-navy)] text-[var(--color-navy)]" : "border-transparent text-[var(--color-ink-muted)] hover:text-[var(--color-ink)]"}`}>
            {filter.label}
            <span className={`font-[var(--font-mono)] text-[9px] px-1.5 py-0.5 rounded ${activeFilter === filter.id ? "bg-[var(--color-navy)] text-white" : "bg-[var(--color-surface)] text-[var(--color-ink-muted)]"}`}>{filter.count}</span>
          </button>
        ))}
      </div>
