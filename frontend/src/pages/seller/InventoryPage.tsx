...
import {
  ...
  type SellerProduct,
  type SellerProductsMeta,
} from "../../api/seller"
import { useUrlTab } from "../../hooks/useUrlTab"

type StockStatus = "in-stock" | "low-stock" | "out-of-stock"
type InventoryTab = StockStatus | "all"

const INVENTORY_TABS: readonly InventoryTab[] = [
  "all",
  "in-stock",
  "low-stock",
  "out-of-stock",
...
const STATUS_CFG: Record<StockStatus, {
  label: string
  color: string
  bg: string
}> = {
...
export default function InventoryPage() {
  ...
  const { activeTab: filter, setActiveTab: setFilter } = useUrlTab(
    INVENTORY_TABS,
    "all",
  )
  ...
  const filtered = items.filter(
    (item) => filter === "all" || getStatus(item) === filter,
  )
  ...
  const localCounts = {
    ...
    "out-of-stock": items.filter((item) => getStatus(item) === "out-of-stock")
      .length,
  }
  const counts = meta?.counts ?? localCounts
    ...
    try {
      const response = await updateSellerInventory(item.productId, {
        ...
        variant_id: item.variant
          ? Number(item.id.split("-").slice(-1)[0])
          : null,
      ...
      setProducts((prev) =>
        prev.map((product) => {
          const productVariant = product.variants.find(
            (variant) => `${product.id}-${variant.id}` === item.id,
          )
          if (productVariant) {
            return {
              ...product,
              variants: product.variants.map((variant) =>
                variant.id === productVariant.id
                  ? { ...variant, stock_quantity: response.data.quantity }
                  : variant,
              ),
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--color-ink-muted)]">
            Track and adjust stock across all your products
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          },
          {
            ...
            sub: "unavailable to buyers",
            color: "var(--color-red)",
          },
        ].map((card) => (
          <div
            key={card.label}
            className="bg-white border border-[var(--color-border)] rounded-sm px-5 py-4"
          >
            <p className="text-xs text-[var(--color-ink-muted)] mb-1">
              {card.label}
            </p>
            <p
              className="font-[var(--font-display)] text-2xl font-[400]"
              style={{ color: card.color ?? "var(--color-ink)" }}
            >
              {card.value}
            </p>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 border border-[var(--color-border)] rounded-sm bg-white px-3 py-2 ml-auto min-w-48 focus-within:border-[var(--color-navy)] transition-colors">
          ...
          <input
            ...
            className="text-sm text-[var(--color-ink)] placeholder-[var(--color-ink-disabled)] focus:outline-none bg-transparent w-full font-[var(--font-body)]"
          />
        </div>
      </div>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface)]">
                {["Product / SKU", "Category", "Available", "Status", ""].map(
                  (heading) => (
                    <th
                      key={heading}
                      className="px-4 py-3 text-left font-[var(--font-mono)] text-[9px] text-[var(--color-ink-muted)] uppercase tracking-widest"
                    >
                      {heading}
                    </th>
                  ),
            ...
            <tbody>
              {filtered.map((item) => {
                ...
                const pct = Math.min(
                  100,
                  (item.quantity / Math.max(1, item.threshold * 2)) * 100,
                )

                return (
                  <tr
                    key={item.id}
                    className="border-b border-[var(--color-border-subtle)] last:border-0 hover:bg-[var(--color-surface)] transition-colors"
                  >
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-[500] text-[var(--color-ink)] truncate max-w-44">
                            {item.productName}
                            {item.variant ? ` - ${item.variant}` : ""}
                          </p>
                          <p className="font-[var(--font-mono)] text-[10px] text-[var(--color-ink-disabled)]">
                            {item.sku}
                          </p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 text-xs text-[var(--color-ink-muted)]">
                      {item.category}
                    </td>
                    <td className="px-4 py-3.5">
                      <div>
                        <span
                          className={`font-[var(--font-mono)] text-base font-[600] ${
                            status === "out-of-stock"
                              ? "text-[var(--color-red)]"
                              : status === "low-stock"
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className="font-[var(--font-mono)] text-[9px] px-2 py-1 rounded"
                        style={{ color: cfg.color, background: cfg.bg }}
                      >
                        {cfg.label}
                      </span>
                      {status === "low-stock" && (
                        <p className="text-[10px] text-[var(--color-amber)] mt-0.5">
                          Min: {item.threshold}
                        </p>
                      )}
                    </td>
                    <td className="px-4 py-3.5">
                      <button
                        ...
                        className="text-xs font-[500] text-[var(--color-navy)] hover:underline cursor-pointer whitespace-nowrap"
                      >
                        Adjust
                      </button>
                    </td>
          ...
          <div className="flex flex-col gap-3 border-t border-[var(--color-border)] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                disabled={page <= 1}
                className="rounded-sm border border-[var(--color-border)] px-3 py-1.5 text-xs disabled:opacity-40"
              >
                Previous
              </button>
                ...
                .map((number, index, pages) => (
                  <span key={number} className="contents">
                    {index > 0 && number - pages[index - 1] > 1 && (
                      <span className="px-1 text-xs text-[var(--color-ink-muted)]">
                        …
                      </span>
                    )}
                    <button
...
function AdjustDialog({
  ...
  item: InventoryItem
  onClose: () => void
  onSave: (item: InventoryItem, newQty: number) => Promise<void>
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white border border-[var(--color-border)] rounded-sm shadow-xl p-6 max-w-md w-full">
        <h3 className="font-[600] text-[var(--color-ink)] mb-1">
          Adjust inventory
        </h3>
        <p className="text-sm text-[var(--color-ink-muted)] mb-4">
          {item.productName}
          {item.variant ? ` - ${item.variant}` : ""}{" "}
          <span className="font-[var(--font-mono)] text-xs">({item.sku})</span>
        </p>
        ...
        <div className="flex items-center gap-3 mb-4 px-4 py-3 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-sm">
          <div>
            ...
            <p className="font-[var(--font-mono)] text-xl text-[var(--color-ink)]">
              {item.quantity}
            </p>
          </div>
          <svg
          ...
          <div>
            ...
            <p
              className={`font-[var(--font-mono)] text-xl font-[600] ${
                preview() === 0
                  ? "text-[var(--color-red)]"
                  : preview() <= item.threshold
                    ? "text-[var(--color-amber)]"
        ...
        <div className="flex gap-2 mb-4">
          {(["add", "subtract", "set"] as const).map((value) => (
            <button
              ...
              className={`flex-1 py-2 text-sm font-[500] rounded-sm border cursor-pointer capitalize transition-colors ${
                mode === value
                  ? "bg-[var(--color-navy)] border-[var(--color-navy)] text-white"
        ...
        <div className="mb-5">
          ...
          <input
            ...
            min="0"
            className="w-full px-3 py-2.5 border border-[var(--color-border)] rounded-sm text-sm text-[var(--color-ink)] focus:outline-none focus:border-[var(--color-navy)] font-[var(--font-body)]"
          />
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 border border-[var(--color-border)] text-sm text-[var(--color-ink-muted)] rounded-sm hover:bg-[var(--color-surface)] cursor-pointer"
          >
            Cancel
          </button>
          <button
            onClick={() => void onSave(item, preview())}
            className="flex-1 py-2.5 bg-[var(--color-navy)] text-white text-sm font-[500] rounded-sm hover:bg-[var(--color-navy-hover)] cursor-pointer"
          >
            Preview adjustment
          </button>
        </div>
