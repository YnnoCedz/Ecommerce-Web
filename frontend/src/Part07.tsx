...
type BuyerSection = "cart" | "wishlist" | "product" | "dashboard" | "reviews";
...
const SECTIONS: { id: BuyerSection; label: string; sublabel: string }[] = [
  ...
  { id: "dashboard", label: "Dashboard", sublabel: "Orders · Wishlist · Notifications" },
  { id: "reviews", label: "Reviews", sublabel: "Rating · Create · Edit · Status" },
];

export default function Part07() {
  const [section, setSection] = useState<BuyerSection>("dashboard");
  ...
  const renderSection = () => {
    ...
    switch (section) {
      ...
      case "dashboard":   return shell(<BuyerDashboardPage />, 3, 34);
