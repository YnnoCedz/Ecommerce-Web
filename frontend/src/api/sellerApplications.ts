...
export async function submitSellerApplication(payload: SellerApplicationPayload) {
  ...
  if (payload.messaging_phone) formData.append("messaging_phone", payload.messaging_phone);
  payload.categories.forEach((categoryId, index) => formData.append(`categories[${index}]`, String(categoryId)));
  ...
  if (payload.business_document_file) formData.append("business_document_file", payload.business_document_file);

  return apiFetch<{ message: string; application: SellerApplicationSummary }>("/seller/applications", {
    method: "POST",
    body: formData,
  });
}
...
export async function fetchSellerApplications(params?: {
  ...
  if (typeof params?.category_id === "number") searchParams.set("category_id", String(params.category_id));
...
export async function viewSellerDocument(id: number) {
  return apiFetch<{ data: { id: number; document_type: string; file_name: string; original_filename: string | null; mime_type: string | null; file_size: number | null; private: boolean; temporary_url: string } }>(`/admin/seller-documents/${id}/view`);
}
